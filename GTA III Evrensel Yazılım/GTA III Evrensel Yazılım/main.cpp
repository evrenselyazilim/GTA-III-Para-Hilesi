...
{
	system("color 0c");
	system("title GTA III Cheat by Evrensel Yazilim");
	
	DWORD address = 0x94139C;
	LPCSTR windowName = "GTA3";
	...
	try
	{
		...
		cout << "GTA III Cheat by Evrensel Yazilim" << endl;
	...
	moneyError:
		...
		{
			cout << endl << "Money amount can only be from 0 to 999.999.999 Try Again";
		}
...
long int readFromMem(DWORD address, LPCSTR windowName)
{
	...
	else
	{
		...
		{
			int status = ReadProcessMemory(processHandle, (LPVOID)address, &value, sizeof(value), 0);
